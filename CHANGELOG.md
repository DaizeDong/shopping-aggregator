...
- **NEW `tools/data_boundary.py`, wired into both hooks and CI** — asserts no DATA path is tracked and that each still ships its schema. It runs **before** `pii_guard`, which is demoted from primary control to backstop: the class declaration catches what the content scan structurally cannot.
