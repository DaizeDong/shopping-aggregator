...
| 京东开放平台 API | ① | 京东联盟 API 仅给推广位/订单，**无公开价格查询接口** | n/a | **✗ 非卖家无法用**，京东不开放价格 API |
| 拼多多开放平台 | ① | 仅商家自家数据 | n/a | **✗ 非卖家无法用** |
...
- **退货窗口（决策级，常被高估）**：**Temu = 购买日起 90 天**（每单首次退货免运，其后 $7.99；请求后 14 天内须寄出否则作废）；**SHEIN ≈ 30 to 45 天**（多源记 35），亦每单一次免运。报告须写「从**购买日**算非收货日」,直邮 1 to 2 周后实际只剩 ~75 to 80 天（[Temu 政策](https://www.temu.com/return-and-refund-policy.html)、[90 vs 35 天](https://www.purchy.app/blog/temu-return-policy-2026)）。
